...
forward_reads <- list.files(raw_fastqs_path, pattern = "_R1.fastq.gz",
                            full.names = TRUE) 
#Intuition check
head(forward_reads)

#Create a vector of reverse reads
reverse_reads <-list.files(raw_fastqs_path, pattern = "_R2.fastq.gz",
                           full.names = TRUE)
#Intuition check
head(reverse_reads)
...
# # Zach experiment
# sapply(forward_reads, function(s) strsplit(basename(s), "_" [1]))
 # First element of forward reads
get_name <- function(s){unlist(strsplit(basename(s), "_"))[1]}

possible_names <-sapply(forward_reads, get_name, USE.NAMES = FALSE)
#Intuition check
head(possible_names)
...
filtered_forward_reads <- file.path(filtered_fastqs_path, 
                                    paste0(possible_names,
...
filtered_reverse_reads <- file.path(filtered_fastqs_path, 
                                    paste0(possible_names, 
...
raw_ASV_table_trimmed <- 
  raw_ASV_table[,nchar(colnames(raw_ASV_table)) %in% 236]
...
write.table(asv_tab, "data/01_DADA2/Angert_analysis/ASV_counts.tsv",
            sep = "\t", quote = FALSE, col.names = NA)
# Write count table with ASV sequence names
write.table(noChimeras_ASV_table,
            "data/01_DADA2/Angert_analysis/ASV_counts_withSeqNames.tsv",
            sep = "\t", quote = FALSE, col.names = NA)
# Write out the fasta file for reference later on for what seq matches what ASV
asv_fasta <- c(rbind(asv_headers, asv_seqs))
# Save to a file!
write(asv_fasta, "data/01_DADA2/Angert_analysis/ASVs.fasta")
...
save(noChimeras_ASV_table,
     file = "data/01_DADA2/Angert_analysis/noChimeras_ASV_table.RData")
save(asv_tab, file = "data/01_DADA2/Angert_analysis/ASV_counts.RData")
...
save(track_counts_df, 
     file = "data/01_DADA2/Angert_analysis/track_read_counts.RData")
```
...
# 
# # Intution check
# stopifnot(asv_tax$ASV == rownames(asv_tax), rownames(asv_tax) == 
#             rownames(asv_tab))
```
