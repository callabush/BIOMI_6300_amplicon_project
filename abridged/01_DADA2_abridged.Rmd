...
# This tries to make any calculations with randomness replicable when rerun
set.seed(567890)
```
...
forward_reads <- list.files(raw_fastqs_path, pattern = "_R1_001.fastq.gz",
                            full.names = TRUE) 
#Intuition check
head(forward_reads)
...
# Look at all the reverse and forward reads
...
```{r prep-filtered-reads, include = FALSE}
# vector of our samples, extract the sample information from our file
samples <- sapply(strsplit(basename(forward_reads), "_"),
...
filtered_forward_reads <- file.path(filtered_fastqs_path,
                                    paste0(samples, "_R1_filtered.fastq.gz"))
...
filtered_reverse_reads <- file.path(filtered_fastqs_path,
                                    paste0(samples, "_R2_filtered.fastq.gz"))
...
This quality plots look a lot better than the previous ones before filterAndTrim. 
By "a lot better", I mean that there are less reads with quality scores below 30. 
...
#Create plot for forward reads errors
forward_error_plot <-
  plotErrors(error_forward_reads, nominalQ = TRUE) + 
  labs(title = "Forward Read Error Model")

#Learn errors for reverse reads
error_reverse_reads <-
  learnErrors(filtered_reverse_reads, multithread = TRUE)

#Create plot for reverse reads errors
reverse_error_plot <-
...
Both the forward and reverse reads seem to fit the error models pretty well.
The gray points align pretty well to the black lines (estimated error rates).
